...
// heroIDByName provides O(1) lookup by name.
var heroIDByName = func() map[string]int {
	m := make(map[string]int, len(heroIds))
	for _, hero := range heroIds {
		m[hero.Name] = hero.ID
	}
	return m
}()

func HeroID(name string) int {
	id, _ := heroIDByName[name]
	return id
}
