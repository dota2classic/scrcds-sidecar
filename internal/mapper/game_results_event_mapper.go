...
func MapGameResults(d models.MatchFinishedOnSRCDS) models.GameResultsEvent {
	// Collect unique party IDs
	parties := uniqueParties(d.Players)

	// Map players
	players := make([]models.PlayerInMatchDTO, 0, len(d.Players))
	for _, p := range d.Players {
		partyIndex := indexOfParty(parties, p.PartyID) + 1 // +1 same as TS version

		player := models.PlayerInMatchDTO{
			...
			LastHits:  p.LastHits,
			Denies:    p.Denies,
			Abandoned: p.Connection == models.DOTA_CONNECTION_STATE_ABANDONED || p.Connection == models.DOTA_CONNECTION_STATE_FAILED,
			Networth:  p.Networth,
	...
	return models.GameResultsEvent{
		MatchID:        d.MatchID,
		Winner:         d.Winner,
		Duration:       d.Duration,
		GameMode:       d.GameMode,
